(function(){
  ...
  function create(root, awardXp){
    ...
    function buildWorkspace(){
      ...
      expressionInput.placeholder = '式やコマンドを入力 (例: integrate(sin(x), x), 5 cm to inch, tetra(2, 3.5) など)';
      ...
      graphMessage.textContent = 'x軸・y軸は自動スケール。複素数は実部を描画します。';
    ...
    function plotGraph(){
      ...
      delete scope.x;
      if (!points.length) {
        graphMessage.textContent = '描画できる点がありません (複素数や未定義が多い可能性があります)。';
        return;
      }
      if (ymax === ymin) {
        ymax += 1;
