(function(){
  function create(root, awardXp, opts){
    ...
    function resetBricks(){
      ...
      for(let c=0;c<cols;c++){
        ...
        for(let r=0;r<rows;r++){
          bricks[c][r] = { x: offLeft + c*(bw+pad), y: offTop + r*(bh+pad), live: 1 };
        }
      }
    }
    resetBricks();
    ...
    function addSparks(x,y,n=8){
      for(let i=0;i<n;i++){
        sparks.push({ x, y, vx:(Math.random()*2-1)*2.4, vy:(Math.random()*-2.2), t: 18 });
      }
    }

    function step(){
      // paddle move (keyboard only)
      ...
      if (ball.y > H + ball.r){
        ...
        ball.x = W/2;
        ball.y = H*0.6;
        ball.dx = cfg.ballSpeed*(Math.random()<0.5?-1:1);
        ball.dy = -cfg.ballSpeed;
      }

      // level clear -> next layout
      if (destroyed > 0) {
        let remain = 0;
        for(let c=0;c<cols;c++) for(let r=0;r<rows;r++) if(bricks[c][r].live) remain++;
        if (remain === 0){
          awardXp && awardXp(cfg.clearBonus, { type:'clear' });
          resetBricks();
        }
      }
      ...
      for(const p of sparks){
        ctx.fillStyle=`rgba(250,250,250,${p.t/18})`;
