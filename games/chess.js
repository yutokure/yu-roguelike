(function(){
  ...
  function create(root, awardXp = () => {}, options = {}){
    ...
    function updateInfo(){
      if (!running){
        const stoppedText = translateText('selection.miniexp.games.chess.status.stopped', '停止中');
        turnLine.innerHTML = `<strong>${escapeHtml(stoppedText)}</strong>`;
      } else {
        const turnLabel = translateText('selection.miniexp.games.chess.status.turnLabel', '手番:');
        const stateText = turn === 'w'
          ? translateText('selection.miniexp.games.chess.status.yourTurn', 'あなたの番です')
          : translateText('selection.miniexp.games.chess.status.aiThinking', 'AIの思考中…');
        turnLine.innerHTML = `<strong>${escapeHtml(turnLabel)}</strong> ${escapeHtml(stateText)}`;
      }
      const scoreLabel = translateText('selection.miniexp.games.chess.status.scoreLabel', 'スコア:');
    ...
    function applyStaticText(){
      const difficultyName = translateText(`selection.miniexp.games.chess.difficultyValue.${difficulty.toLowerCase()}`, difficulty);
      title.textContent = translateText('selection.miniexp.games.chess.title', 'チェス');
      diffTag.textContent = translateText('selection.miniexp.games.chess.difficultyTag', () => `難易度: ${difficultyName}`, { value: difficultyName });
      resetBtn.textContent = translateText('selection.miniexp.games.chess.controls.restart', 'リスタート');
    }
    ...
    function evaluateGameEnd(lastMover){
      ...
      if (moves.length === 0){
        if (inCheck){
          if (opponent === 'b'){
            ...
            setMessage({ key: 'selection.miniexp.games.chess.messages.checkmateWin', fallback: 'チェックメイト！勝利しました。' });
          } else {
            setMessage({ key: 'selection.miniexp.games.chess.messages.checkmateLoss', fallback: 'チェックメイトを受けました…' });
          }
        } else {
          awardXp(DRAW_EXP, { reason: 'draw' });
          setMessage({ key: 'selection.miniexp.games.chess.messages.stalemate', fallback: 'ステイルメイト。引き分けです。' });
        }
        running = false;
        resetSelection();
        updateInfo();
      ...
      if (halfMoveClock >= 100 || totalMoves >= 200 || insufficientMaterial(board)){
        awardXp(DRAW_EXP, { reason: 'draw' });
        setMessage({ key: 'selection.miniexp.games.chess.messages.draw', fallback: '引き分け扱いになりました。' });
        running = false;
        updateInfo();
        return true;
      }
    ...
    function executeMove(move, mover){
      ...
      if (inCheck){
        if (moverColor === 'w'){
          ...
          setMessage({ key: 'selection.miniexp.games.chess.messages.playerCheck', fallback: 'チェック！' });
        } else {
          setMessage({ key: 'selection.miniexp.games.chess.messages.playerInCheck', fallback: 'チェックされています！' });
        }
    ...
    function handleClick(ev){
        ...
        if (move){
          if (move.promote){
            const choice = window.prompt(
              translateText('selection.miniexp.games.chess.prompts.promotion', '昇格する駒を選んでください (Q/R/B/N)'),
              'Q'
            );
            if (choice){
      ...
      if (piece && pieceColor(piece) === 'w'){
        ...
        setMessage({ key: 'selection.miniexp.games.chess.messages.selectMove', fallback: '移動するマスを選択してください' });
      } else {
        resetSelection();
      }
      drawBoard();
